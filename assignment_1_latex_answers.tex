...
\small{Figure 6. The emprical ranges of $Z_n$ over 500 realisations at n=10 and n=100. The whiskers are set at $1.5 \times IQR$ where IQR is the interquartile range.}
\end{figure}
...
Each individual can have the starting type of any other individual. If we consider a vector that contains all of the unique starting states, $b_0 = [X_0(1), X_0(2), . . . , X_0(L)]$, then in the state space, each individual can have a type of any element in that vector. This gives a total state space of size $L^2$. In mathematical notation:
